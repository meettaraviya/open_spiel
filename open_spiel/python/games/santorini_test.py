...
from open_spiel.python.games import santorini
...
class SantoriniTest(absltest.TestCase):

  def test_can_create_game_and_state(self):
    """Checks we can create the game and a state."""
    game = santorini.SantoriniGame()
    state = game.new_initial_state()
    self.assertEqual(str(state), "\n".join([" ".join(["⚬ "] * santorini._NUM_COLS)] * santorini._NUM_ROWS))
  ...
  def test_game_from_cc(self):
    ...
    game = pyspiel.load_game("python_santorini")
  ...
  def test_cloned_state_matches_original_state(self):
    ...
    state.apply_action(158)
    state.apply_action(390)
